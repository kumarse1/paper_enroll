        ...
        if st.button("🚀 Submit & Process"):
            with st.spinner("⏳ Processing document..."):
                ...
                tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(
                    ["📄 Uploaded Document", "🖼️ Cleaned Image", "🔍 Extracted Fields (Table)", "📊 Confidence Scores", 
                     "📝 Full JSON Output", "✅ Validation Results", "📜 OCR Raw Log"]
                )
                ...
                with tab6:
                    ...
                    for page in all_results:
                        for word in page["real_values"]:
                            extracted_data[word["word"]] = str(word["confidence"])  # ✅ Convert values to string before validation

                    # ✅ If extracted_data is empty, fallback to sample test values
                    if not extracted_data:
                        st.warning("⚠️ No OCR-extracted values found. Using default test values.")
                        extracted_data = {
                            "First Name": "John",
                            "Last Name": "Doe",
                            "SSN": "123-45-678",
                ...
                with tab7:
                    st.subheader("📜 Raw OCR Log")
                    ocr_log_text = json.dumps(all_results, indent=4)
                    st.text_area("Raw OCR Extracted Data:", ocr_log_text, height=300)
